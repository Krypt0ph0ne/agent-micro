...
final class DeviceService {
    ...
    @discardableResult
    func validate(profile: MacropadProfile, keyboardLayout: KeyboardLayout = .usANSI, layerSwitchControl: HardwareControl? = nil) -> ProcessResult? {
        if currentDevice?.isCodexPadFirmware == true {
            do {
                let packets = try codexPadEncoder.uploadPackets(profile: profile, layout: keyboardLayout, layerSwitchControl: layerSwitchControl)
    ...
    @discardableResult
    func upload(profile: MacropadProfile, keyboardLayout: KeyboardLayout = .usANSI, layerSwitchControl: HardwareControl? = nil) -> ProcessResult? {
        ...
        let validation = validate(profile: profile, keyboardLayout: keyboardLayout, layerSwitchControl: layerSwitchControl)

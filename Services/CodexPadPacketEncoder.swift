...
struct CodexPadPacketEncoder {
    ...
    ///
    /// The device protocol has no pulse-floor byte. The persisted baseline is
    /// therefore a firmware-native pulse; while the app is connected,
    /// `CodexPadLEDFeedbackService` renders configured non-zero ranges.
    ...
    func bindingPacket(action: KeyboardAction, control: HardwareControl, layout: KeyboardLayout = .usANSI) throws -> [UInt8] {
        ...
        if action.kind == .hostEvent || action.kind == .layerSwitch || action.kind == .profileSwitch || action.kind.isAgent {
            packet[5] = 3
            return finalized(packet)
        }

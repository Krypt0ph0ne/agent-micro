...
Agent Micro is a local native macOS 14+ SwiftUI application for configuring a connected 3×2 CH57x macro pad with one encoder. It is an independent project inspired by calm desktop tooling—not an official OpenAI product, and it contains no OpenAI logos or copied assets.
...
- Supports the custom CH552 Agent Micro firmware at `0x4249:0x4287`: nine live-reconfigurable keyboard/media bindings, real press/release hold actions, app-only events, firmware status, emergency release-all, and six independent RGB LEDs with off, steady, blink and pulse effects.
...
`script/build_and_run.sh` always stops an old `CodexPad` process, builds the locally checked-out Rust helper, builds the SwiftPM target, stages `dist/Agent Micro.app`, and launches it. Its modes are `run`, `--debug`, `--logs`, `--telemetry`, and `--verify`.
...
The confirmed 0x8890 hardware limits a keyboard sequence to five chords and does not support the helper's delay option. Deep links and local shell commands are stored as non-uploadable profile metadata; Agent Micro will not execute them automatically.
...
The visible app is intentionally limited to one **Codex** profile and three encoder actions. F22/F23/F24 are private hardware triggers. Rotation is translated to the configured Codex shortcuts F18/F19 and changes reasoning directly without opening a picker. The first encoder press opens the Model Picker; the next press sends Escape to close it. Agent Micro performs no model navigation or confirmation. Older unchanged encoder defaults are migrated automatically.
